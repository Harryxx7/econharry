  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-4">
      <div className="flex items-center gap-2 mb-4 text-sm text-slate-500">
        <Search size={14} />
        <span>找到 <strong className="text-slate-800">{results.length}</strong> 条结果，关键词："{query}"</span>
      </div>
        ...
        {results.map(note => {
          ...
          return (
            <button
              ...
              className="w-full text-left p-4 bg-white rounded-xl border border-slate-200 hover:border-indigo-300 hover:shadow-sm transition-all"
            >
